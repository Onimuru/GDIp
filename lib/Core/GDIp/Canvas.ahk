...
static CreateCanvas(x, y, width, height, windowProc := False, windowClassName := "Canvas", windowClassStyles := 0x00000000, title := "Title", extendedWindowStyles := 0x00000000, windowStyles := 0x00000000, smoothing := 4, interpolation := 7) {
	if (windowProc && !(windowProc is Func || windowProc is Closure)) {
		...
		windowProc := DefaultWindowProc

		DefaultWindowProc(hWnd, uMsg, wParam, lParam) {
			switch (uMsg) {
				case 0x201:  ;? 0x201 = WM_LBUTTONDOWN
					parent := DllCall("GetParent", "Ptr", hWnd, "Ptr")

					PostMessage 0xA1, 2, , , (parent != A_ScriptHwnd && parent != 0) ? (parent) : (hWnd)
			}
	...
	wndClassEx := Structure.CreateWndClassEx(windowClassStyles, CallbackCreate(windowProc, "F"), 0, 0, DllCall("GetModuleHandle", "Ptr", 0, "Ptr"), 0, 0, 0, 0, StrPtr(windowClassName), 0)
	...
	if (!(extendedWindowStyles)) {
		extendedWindowStyles := WS_EX_LAYERED | WS_EX_NOACTIVATE | WS_EX_TOOLWINDOW | WS_EX_TOPMOST | WS_EX_TRANSPARENT
	}
	...
	(instance := this.Canvas()).Handle := hWnd  ;* Save a handle to the window in order to update it.
...
class Canvas {
	...
	Update(x := unset, y := unset, width := unset, height := unset, alpha := unset) {
		...
		if (IsSet(x)) {
			if (IsSet(y)) {
				point.NumPut(0, "UInt", x, "UInt", y)
			}
			else {
				point.NumPut(0, "UInt", x)
			}
		}
		else if (IsSet(y)) {
			point.NumPut(4, "UInt", y)
		}

		if (IsSet(width)) {
			if (IsSet(height)) {
				size.NumPut(0, "UInt", width, "UInt", height)
			}
			else {
				size.NumPut(0, "UInt", width)
			}
		}
		else if (IsSet(height)) {
			size.NumPut(4, "UInt", height)
		}

		if (IsSet(alpha)) {
			blend.NumPut(2, "UChar", alpha)
		}
