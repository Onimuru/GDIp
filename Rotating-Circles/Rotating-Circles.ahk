...
Class Ball {
	__New(_x, _y, _offset, _size, _angle) {
		C := Format("{:02X}{:02X}{:02X}", Round((C := HSV_Convert2RGB(Abs(_angle/360)))[1]*255), Round(C[2]*255), Round(C[3]*255))

		this.Cx := this.Ox := _x + _offset
			, this.Cy := this.Oy := _y + _offset
			, this.Dx := (-_x*2)/vDiameter
			, this.Dy := (-_y*2)/vDiameter

		this.S := _size
		this.B := [Gdip_BrushCreateSolid("0xFF" . C), Gdip_BrushCreateSolid("0xCC" . C), Gdip_BrushCreateSolid("0x99" . C), Gdip_BrushCreateSolid("0x66" . C), Gdip_BrushCreateSolid("0x33" . C)]
		this.P := []
		this.M := -3.5
		this.E := (Abs(this.A := _angle) < 180)
	}
	...
	Update() {
		...
		If (D >= vRadius*0.95 - 1) {
			...
			If (this.E)
				Balls.Push(new Ball(-(this.Ox - vRadius), -(this.Oy - vRadius), vRadius, vRadius/10, this.A + 180*vClockwise))
					, this.E := 0
		}
