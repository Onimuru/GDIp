...
Global oCanvas := new GDIp.Canvas([A_ScreenWidth - v*3.5 + 5, v*.5 + 5, v*2 + 10, v*2 + 10], "-Caption +AlwaysOnTop +ToolWindow +OwnDialogs +E0x20")
	...
	, vSolarSystem := {"h": oCanvas.Position.Width/2
		, "k": oCanvas.Position.Height/2
...
vSolarSystem.Planets.Push(new Planet("Sun", Min(oCanvas.Position.Width, oCanvas.Position.Height)/10, 0, 0, 0, vSolarSystem))  ;? (Name, Diameter, OrbitAngle, OrbitRadius, OrbitRevolution (in days), Parent)
Loop, % Math.Random(1, 5) {
	vSolarSystem.Planets.Push(new Planet(A_Index, Math.Random(10, vSolarSystem.Planets[0].Diameter[0]*1.35), Math.Random(0, 360), Math.Random(vSolarSystem.Planets[0].Diameter[0], Min(oCanvas.Position.Width/2, oCanvas.Position.Height/2)), Math.Random(250, 2500), vSolarSystem.Planets[0]))
...
#If (WinActive(A_ScriptName) || WinActive("GDIp_Canvas.ahk") || WinActive("Geometry.ahk"))
...
Update:
	If (QueryPerformanceCounter_Passive()) {
		...
		oCanvas.DrawString(oBrush[0], Round(vSolarSystem.Date) . " days", "Bold r4")
		For i, v in vSolarSystem.Planets {
			v.Update(vSolarSystem.Date)
		}

		If (vSolarSystem.SpeedRatio != 1) {
			oCanvas.DrawString(oBrush[0], Round(vSolarSystem.SpeedRatio, 1) . "x", "x" . oCanvas.Position.Width - 35 . "Bold r4")
		}
...
Class Planet {
	__New(vName, vDiameter, vOrbitAngle, vOrbitRadius, vOrbitRevolution, vParent) {
		...
		this.Brush := (vName == "Sun" ? new GDIp.Brush("0xFF" . ["FFFF00", "00FFFF", "FFFFFF"][Math.Random(0, 2)]) : new GDIp.LinearGradientBrush([vDiameter/2, vDiameter/2, vDiameter, vDiameter], [Color.Random(), Color.Random()], vOrbitAngle))
	}
	...
	Draw(vDate) {
		...
		oCanvas.DrawString(oBrush[0], this.Name . ((this.Parent.Name == "Sun") ? (" (" . Round(Mod((vDate/this.Orbit.Revolution)*360, 360)) . "°)") : ("")), "x" . this.h + this.Diameter[1]/2 . "y" . this.k - this.Diameter[1]/2 . "Bold r4")
	}
}