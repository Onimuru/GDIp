...
Update:
	...
	if (vDegrees <= 90)
		vOffset_x := vDiameter*Sin(vRadians), vOffset_y := 0

	else if (vDegrees <= 180)
		vOffset_x := (vDiameter*Sin(vRadians)) - (vDiameter*Cos(vRadians)), vOffset_y := -vDiameter*Cos(vRadians)

	else if (vDegrees <= 270)
		vOffset_x := -(vDiameter*Cos(vRadians)), vOffset_y := -(vDiameter*Cos(vRadians)) - (vDiameter*Sin(vRadians))

	else
		vOffset_x := 0, vOffset_y := -vDiameter*Sin(vRadians)
